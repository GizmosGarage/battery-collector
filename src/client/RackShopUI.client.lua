--[[
	...
	Where GPUs actually get INSTALLED now. Buying at the GPU Shop
	(ShopUI.client.lua) only ever puts a card into STORAGE -- picking which
	physical Server_Rack (and which of its 4 slots) it fills is done by
	walking up to that specific rack. Get close to one and a panel appears
	listing every GPU type you have in storage; click "Equip Here" on one
	and the server puts it in the first empty, unlocked slot THAT rack
	owns.
...
local function rackStatus(rackIndex)
...
local function buildRackPanel(rackIndex)
	...
	-- One row per GPU TYPE in the catalog -- same fixed-row-count idea as
	-- the GPU Shop's catalog, so owning three Basics still shows one row.
	local rows = {}
	for i, gpu in GPUs.catalog do
		rows[gpu.id] = makeRowFrame(panel, 2 + i, function()
	...
	local function refresh()
		...
		local status = rackStatus(rackIndex)
